...
{
	if(x>=0&&x<m&&y>=0&&y<n){
		if(vis[x][y]==false&&(ary[x][y]=='.'||ary[x][y]=='T'))
		//I can not think ary[x][y]=='T' is important conditon
			return true;
	}		
	return false;
}
	...
	while(!que.empty())
	{
		...
		for(int i = 0; i<2; i++)
			if(judge(tp.x+deX[i], tp.y)){
				que.push(Node(tp.x+deX[i], tp.y, tp.layer+1));  
				//use last layer to add 1 that is perfect
			}
		for(int j = 0; j<2; j++)
